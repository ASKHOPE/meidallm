...
export function renderPublishView(pid: string): string {
    ...
    <div class="flex-grow bg-background border border-text-main/15 p-6 rounded-2xl">
        <h3 class="text-xl font-semibold text-text-main font-outfit mb-2">Publish & Schedule Campaign</h3>
        ...
        <div class="flex flex-col gap-4">
            <div>
                ...
                <select id="publish-draft-select" class="w-full bg-panel-hover border border-text-main/15 p-3 rounded-xl text-text-main text-sm focus:outline-none focus:border-text-main cursor-pointer">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="publish-datetime" type="datetime-local" class="w-full bg-panel-hover border border-text-main/15 p-3 rounded-xl text-text-main text-sm focus:outline-none focus:border-text-main cursor-pointer">
                </div>
                <div class="flex items-end">
                    <button onclick="window.schedulePost('${pid}')" class="w-full py-3.5 bg-text-main text-background font-bold text-sm rounded-xl hover:bg-text-main/90 transition-all cursor-pointer shadow-sm">
                        Schedule Release
                    </button>
                </div>
            </div>
    ...
    <div class="flex-grow bg-background border border-text-main/15 p-6 rounded-2xl">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h3 class="text-xl font-semibold text-text-main font-outfit">Release Calendar</h3>
                <p class="text-xs text-text-muted">June 2026 editorial distribution schedules.</p>
            </div>
            <div class="text-xs font-bold text-text-main uppercase tracking-wider bg-panel-hover px-3 py-1.5 rounded-lg border border-text-main/15">June 2026</div>
        </div>
    ...
    <div class="fade-in flex flex-col gap-6 text-text-main">
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="w-full lg:w-80 bg-background border border-text-main/15 rounded-2xl p-6 flex flex-col shrink-0">
                <h3 class="text-lg font-semibold text-text-main font-outfit mb-4">Distribution Queue</h3>
                <div id="publish-queue-list" class="flex flex-col gap-3 max-h-[450px] overflow-y-auto pr-1">
                    ${projectSchedules.map(s => {
                        const isPublished = s.status === 'published';
                        const statusClass = isPublished ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20';
                        return `
                        <div class="bg-panel-hover/30 border border-text-main/10 rounded-xl p-4 flex flex-col gap-2 relative">
                            ...
                            <h4 class="font-medium text-text-main text-xs truncate">${sanitizeHTML(s.title)}</h4>
                            <div class="text-[10px] text-text-muted flex gap-1.5 flex-wrap">
                                ${s.channels.map(c => `<span class="bg-panel-hover border border-text-main/10 px-1.5 py-0.5 rounded">${c}</span>`).join('')}
                            </div>
                            ${!isPublished ? `
...
if (typeof window !== 'undefined') {
    ...
    w.setPublishViewMode = (mode: string) => {
        localStorage.setItem('meidallm_publish_viewmode', mode);
        // trigger render
        state.projects = [...state.projects]; // force notifyStateChange
    };
